 ...
 //load subdesignation
  ...
  openEditPopup(type: 'department' | 'designation' | 'subdesignation', index: number): void {
    ...
    if (type === 'department') {
      ...
      this.newItemStatus =
        (dept?.status + '').toLowerCase() === 'active' || dept?.status === true
          ? 'active'
          : 'inactive';
    } else if (type === 'designation') {
      const desg = this.designations[index];
      this.newItemTitle = desg?.designation_name || '';
      this.selectedDepartmentId = desg?.departmentId || desg?.dept_id || null; // ✅ Preselect department
      this.newItemStatus =
        (desg?.status + '').toLowerCase() === 'active' || desg?.status === true
          ? 'active'
          : 'inactive';
  ...
  updateItem(): void {
    ...
    if (this.editingType === 'department') {
      ...
      const payload = {
        dept_id: dept?._id || dept?.dept_id || dept?.id,
        dept_name: this.newItemTitle,
        status: this.newItemStatus,
      };
      ...
      const payload = {
        designation_id: desg?._id || desg?.desg_id || desg?.id,
        designation_name: this.newItemTitle,
        departmentId: this.selectedDepartmentId,
        status: this.newItemStatus,
      };
